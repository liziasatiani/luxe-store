    ...
    <div className="space-y-6">
      ...
      <AnimatePresence>
        {showForm && (
          ...
          >
            <h2 className="font-semibold text-surface-900 dark:text-white">{t("newAddress")}</h2>
            <div className="grid grid-cols-2 gap-4">
              <Input label={t("label")} value={form.label} onChange={e => set("label", e.target.value)} placeholder={t("labelPlaceholder")} />
              <div />
              <Input label={t("firstName")} value={form.firstName} onChange={e => set("firstName", e.target.value)} />
              <Input label={t("lastName")} value={form.lastName} onChange={e => set("lastName", e.target.value)} />
              <div className="col-span-2">
                <Input label={t("addressLine")} value={form.line1} onChange={e => set("line1", e.target.value)} />
              </div>
              <div className="col-span-2">
                <Input label={t("apartment")} value={form.line2} onChange={e => set("line2", e.target.value)} />
              </div>
              ...
              <Input label={t("postalCode")} value={form.postalCode} onChange={e => set("postalCode", e.target.value)} />
              <Input label={t("country")} value={form.country} onChange={e => set("country", e.target.value)} />
            </div>
            <Checkbox id="default" label={t("setDefault")} checked={form.isDefault} onChange={e => set("isDefault", e.target.checked)} />
            <div className="flex gap-3">
              <Button onClick={handleSave} loading={saving} variant="gold">{t("save")}</Button>
              <Button onClick={() => { setShowForm(false); setForm(BLANK); }} variant="outline">{t("cancel")}</Button>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      {loading ? (
        ...
        <div className="text-center py-16 rounded-2xl border border-dashed border-surface-200 dark:border-surface-700">
          ...
          <p className="text-surface-500">{t("noAddresses")}</p>
        </div>
      ) : (
            ...
            <div key={addr.id} className="flex items-start justify-between gap-4 bg-white dark:bg-surface-900 rounded-2xl border border-surface-100 dark:border-surface-800 p-5">
              <div>
                <div className="flex items-center gap-2 mb-1">
                  ...
                  {addr.isDefault && <span className="text-xs px-2 py-0.5 rounded-full bg-brand-50 dark:bg-brand-900/20 text-brand-600 dark:text-brand-400 font-medium">{t("default")}</span>}
                </div>
