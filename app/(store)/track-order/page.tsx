...
export default function TrackOrderPage() {
  ...
  const handleTrack = async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : "Order not found. Please check your details.");
      setOrder(null);
    } finally {
      setLoading(false);
    }
    ...
    <>
      <div className="bg-surface-50 dark:bg-surface-900/50 border-b border-surface-100 dark:border-surface-800 py-14">
        <Container className="text-center">
          <h1 className="font-display text-5xl text-surface-900 dark:text-white mb-3">Track Your Order</h1>
          <p className="text-surface-500">Enter your order number and email to track your delivery.</p>
        </Container>
      </div>
      <Container className="py-16 max-w-2xl">
        <div className="bg-white dark:bg-surface-900 rounded-2xl border border-surface-100 dark:border-surface-800 p-8 space-y-5 mb-8">
          <Input label="Order Number" placeholder="e.g. LXS-ABC123" value={orderNumber} onChange={e => setOrderNumber(e.target.value)} onKeyDown={e => e.key === "Enter" && handleTrack()} />
          <Input label="Email Address" type="email" placeholder="Email used when ordering" value={email} onChange={e => setEmail(e.target.value)} onKeyDown={e => e.key === "Enter" && handleTrack()} />
          {error && <p className="text-sm text-red-500">{error}</p>}
          <Button onClick={handleTrack} loading={loading} variant="gold" size="lg" fullWidth leftIcon={<Search size={18} />}>Track Order</Button>
        </div>
        ...
        {order && (
          <div className="space-y-6">
            <div className="bg-white dark:bg-surface-900 rounded-2xl border border-surface-100 dark:border-surface-800 p-6">
              <div className="flex items-center justify-between mb-6">
                <div>
                  ...
                  <p className="text-sm text-surface-400 mt-0.5">Placed {formatDate(order.createdAt)}</p>
                </div>
              ...
              {order.trackingNumber && (
                <div className="mt-5 p-3 rounded-xl bg-brand-50 dark:bg-brand-900/20 border border-brand-100 dark:border-brand-800">
                  <p className="text-sm text-surface-600 dark:text-surface-400">
                    Tracking: {order.trackingUrl ? <a href={order.trackingUrl} target="_blank" rel="noopener noreferrer" className="font-mono text-brand-500 font-bold">{order.trackingNumber}</a> : <span className="font-mono font-bold">{order.trackingNumber}</span>}
                  </p>
                </div>
              )}
            </div>
            <div className="bg-white dark:bg-surface-900 rounded-2xl border border-surface-100 dark:border-surface-800 p-6 space-y-3">
              <h3 className="font-semibold text-surface-900 dark:text-white">Items Ordered</h3>
            ...
            {order.shippingName && (
              <div className="bg-white dark:bg-surface-900 rounded-2xl border border-surface-100 dark:border-surface-800 p-6">
                <h3 className="font-semibold text-surface-900 dark:text-white mb-3">Shipping To</h3>
