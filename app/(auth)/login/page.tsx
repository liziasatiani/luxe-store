    ...
    <div className="min-h-screen flex items-center justify-center px-4 py-12">
      <div className="w-full max-w-md">
        ...
        <div className="bg-white dark:bg-surface-900 rounded-2xl border border-surface-100 dark:border-surface-800 p-8 space-y-5">
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            <div className="flex justify-end">
              <Link href="/forgot-password" className="text-sm text-brand-500 hover:text-brand-600">{t("forgotPassword")}</Link>
            </div>
            <Button type="submit" variant="gold" size="lg" fullWidth loading={loading} leftIcon={<LogIn size={18} />}>{t("signIn")}</Button>
          </form>
          <Divider label={t("orContinueWith")} />
          <Button onClick={() => signIn("google", { callbackUrl: redirect })} variant="outline" size="lg" fullWidth>
        ...
        <p className="text-center mt-6 text-sm text-surface-500">
          {t("noAccount")}{" "}<Link href="/register" className="text-brand-500 hover:text-brand-600 font-medium">{t("createAccount")}</Link>
        </p>
      </div>
    </div>
  );
