    ...
    <div className="min-h-screen flex items-center justify-center px-4 py-12">
      <div className="w-full max-w-md">
        <div className="text-center mb-10">
          ...
          <h1 className="font-display text-3xl text-surface-900 dark:text-white mt-6 mb-2">Welcome back</h1>
        ...
        <div className="bg-white dark:bg-surface-900 rounded-3xl shadow-luxury-lg border border-surface-100 dark:border-surface-800 p-8">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            <Input
              id="password" label="Password" type={showPw ? "text" : "password"} placeholder="••••••••"
              error={errors.password?.message}
              rightIcon={<button type="button" onClick={() => setShowPw(p => !p)}>{showPw ? <EyeOff size={16} /> : <Eye size={16} />}</button>}
              {...register("password")}
            />
            <div className="flex justify-end">
              <Link href="/forgot-password" className="text-sm text-brand-500 hover:text-brand-600">Forgot password?</Link>
            </div>
            <Button type="submit" loading={loading} variant="gold" size="lg" fullWidth leftIcon={<LogIn size={18} />}>
              Sign In
            </Button>
          </form>
