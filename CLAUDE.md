...
- **React hydration error #418:** Fixed 2026-09-13 — root cause was BottomTabBar reading Zustand persist cart items and session-based account link without a `mounted` guard. Added `useState(false)` + `useEffect` guard, same pattern as Navbar.
- **TheStandardClient "See all products" link:** Currently redirects /products → /beauty (temporary fix, 2026-09-13). Ideally a proper /products index page listing all products across categories should be built, or the link text updated to "Browse Beauty" + "Browse Tech" separately.
...
[2026-09-18] i18n [FR audit Phase 7] FIX [app/globals.css] — Reduced .btn-cart letter-spacing from 0.1em to 0.04em inside @media (max-width: 768px). French/Spanish "AJOUTER AU PANIER" label was clipped at the last character in 2-column mobile product card grid at 375px. Saving ~11px per character is enough to fit the full label at all mobile viewports.
[2026-09-18] i18n [FR audit Phase 3–5] [app/(store)/beauty/page.tsx, tech/page.tsx, beauty/[slug]/page.tsx, tech/[slug]/page.tsx, app/(auth)/login/layout.tsx, register/layout.tsx, forgot-password/layout.tsx, reset-password/layout.tsx, app/(store)/wishlist/layout.tsx, contact/layout.tsx, faq/layout.tsx, track-order/layout.tsx, checkout/layout.tsx, cart/layout.tsx, account/layout.tsx, account/addresses/layout.tsx, account/orders/page.tsx, account/orders/[id]/page.tsx, checkout/success/page.tsx, brands/page.tsx] — Converted all static `export const metadata` with hardcoded English titles to async `generateMetadata()` using getTranslations. Added 7 new translation keys across all 4 languages (pages.checkout.title, pages.cart.title, pages.account.title, pages.accountAddresses.title, pages.accountOrders.title, pages.accountOrderDetail.title, pages.checkoutSuccess.title). Beauty/tech parent pages use tNav("beauty")/tNav("tech"); subcategory pages use tCat.raw(slug) with cat.name fallback.
[2026-09-18] i18n [FR audit Phase 1–2] [app/(store)/contact/page.tsx, messages/fr|en|es|ka.json] — Converted hardcoded "9AM – 6PM GET" / "10AM – 4PM GET" business hours to locale-aware translation keys. FR format: "9h – 18h GET" / "10h – 16h GET". Added monFriHours and saturdayHours keys to pages.contact namespace in all 4 languages.

[2026-09-13] FIX [app/(store)/checkout/page.tsx] — Removed all Cash on Delivery logic. COD_MAX_GEL, codAvailable, paymentMethod state, Banknote icon, and COD UI button all removed. Stripe is now the only payment method, consistent with CLAUDE.md ("No COD — removed all COD references"). placeOrder always sends paymentMethod:"STRIPE" and always redirects to Stripe Checkout.
[2026-09-13] FIX [next.config.ts] — Added https://www.googletagmanager.com to CSP script-src so GA4 tags (G-F8J8K7CG0Q) can load. Previously blocked by CSP, causing silent analytics failure.
[2026-09-13] FIX [app/layout.tsx] — Removed stale Unsplash preload link (photo-1515886657613-9f3515b0c78f) from <head>. Image is no longer used in any component; hero now uses different Unsplash photos.
[2026-09-13] FIX [next.config.ts] — Added redirect: /products → /beauty (temporary). TheStandardClient "See all products →" link was pointing to /products which has no index page (only /products/[slug] exists). Redirects to /beauty as the closest all-products equivalent.
[2026-09-13] i18n [components/home/TheEditSection.tsx] — Translated hardcoded "Add to Cart" and "Sale" labels using getTranslations("product") and getTranslations("common"). Server component — useTranslations() not available here. Keys used: product.addToCart, common.sale.
[2026-09-13] i18n [components/layout/Navbar.tsx, messages/ka|en|fr|es.json] — Translated announcement bar items (ann1–ann6) in all 4 languages. Previously hardcoded English.
[2026-09-13] i18n [components/layout/Footer.tsx, messages/ka|en|fr|es.json] — Translated all footer nav link labels and column headings (Shop/Company/Support, Technology/Beauty/New Arrivals/All Brands/About Us/Contact/Careers/Shipping/Returns/Terms/Privacy/Cookie Policy). Added missing keys: footer.support, footer.careers, footer.cookiePolicy in all 4 languages.
[2026-09-13] FIX [components/layout/BottomTabBar.tsx] — Added mounted guard to prevent React hydration error #418. Cart count and session-based account link were read from Zustand persist store without guard, causing SSR/client mismatch.
