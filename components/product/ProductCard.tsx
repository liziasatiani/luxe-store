...
export function ProductCard({ product, index = 0, variant = "default" }: ProductCardProps) {
  ...
  const handleAddToCart = (e: React.MouseEvent) => {
    e.preventDefault();
    addItem(product);
    toast.success(t("addToCart"), { icon: "🛍️", style: { borderRadius: "12px" } });
  };
      ...
      >
        <Link href={`/products/${product.slug}`} className="group block">
          <div className="relative overflow-hidden rounded-2xl bg-surface-50 dark:bg-surface-800 aspect-[3/4] mb-4">
            ...
            <div className="absolute top-3 right-3 flex flex-col gap-2 opacity-0 group-hover:opacity-100 transition-all duration-300 translate-x-2 group-hover:translate-x-0">
              <button onClick={handleWishlist}
                className={cn("w-9 h-9 rounded-full flex items-center justify-center shadow-lg backdrop-blur-sm transition-colors",
                  isWishlisted ? "bg-red-500 text-white" : "bg-white/90 dark:bg-surface-900/90 text-surface-700 hover:bg-red-50 hover:text-red-500")}>
                <Heart size={15} fill={isWishlisted ? "currentColor" : "none"} />
              </button>
              <button onClick={handleAddToCart} disabled={product.stockStatus === "OUT_OF_STOCK"}
                className="w-9 h-9 rounded-full bg-white/90 dark:bg-surface-900/90 flex items-center justify-center shadow-lg backdrop-blur-sm text-surface-700 hover:bg-surface-900 hover:text-white transition-colors disabled:opacity-50">
                <ShoppingBag size={15} />
              </button>
            </div>
          ...
          <div className="space-y-1.5">
            {product.brand && <p className="text-xs text-surface-400 uppercase tracking-wider">{product.brand.name}</p>}
            <h3 className="font-medium text-surface-900 dark:text-white line-clamp-2 text-sm leading-snug group-hover:text-brand-500 transition-colors">
              {product.name}
            </h3>
            ...
            <div className="flex items-center gap-2 pt-0.5">
              ...
              {comparePrice && comparePrice > price && (
                <span className="text-sm text-surface-400 line-through">{formatPrice(comparePrice)}</span>
              )}
            </div>
